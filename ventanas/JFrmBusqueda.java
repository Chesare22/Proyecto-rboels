...
=======
    private void imprimirDatos(TextoBuscar text) {
        ...
        System.out.println("");
>>>>>>> 160f3ec6bf103bd5de488413ddc8c30ab3abb408
    }
    ...
    private class TransicionBuscar implements MouseListener {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (indices.length == 0) {
                //Mostrar mensaje de que no se seleccionó algo para buscar
                JOptionPane.showMessageDialog(null, "No se ha llenado ningun campo de busqueda, por favor llene uno");
            } else {
                for (int i = 0; i < textos.length; i++) {
                    indices[i] = textos[i].buscar();
                }
                ...
                if (indices.length == 1) {
                    ...
                    Egresado[] egresados = JFrmInicio.obtenerEgresados(indices[0]);//Obtenemos los egresados que coinciden con la búsqueda

                    if (egresados.length == 0) {//Si no se han encntrado coincidencias, se manda un mensaje
                        
                        JOptionPane.showMessageDialog(null, "No se han encontrado coincidencias con la búsqueda");
                        
                    } else {//Si se encontraron coincidencias, entonces de manda a la otra ventana con los resultados de la búsqueda
